...
C
      INTEGER*4 IHELP(15,19),IHED(15,3),KRAK(20),ASTAT,ESTAT
C
      CHARACTER*60 CHELP(19),CHED(3)
C
      DIMENSION VAL(4),DATAA(4),DATAE(4)
C
      DIMENSION THCM3(2),E3(2),CMTOLB(2)
      DIMENSION JRAK(10),A(4),JATA(4),JATZ(4),IFLG(3)
...
C
  ...
  21  FORMAT (/25H LAB BOMBARDING ENERGY = ,F10.5,39H MEV IS BELOW REACT
     1ION THRESHOLD ENERGY,F6.1)
  22  FORMAT (25H LAB BOMBARDING ENERGY = ,F10.5,69H MEV IS ABOVE REACTI
     1ON THRESHOLD ENERGY,BUT TOO LOW FOR THETA LAB = ,F10.5,F6.1)
C
...
C
  ...
  30  FORMAT(1H ,10A4)
      ...
      END
C$PROG DMILV
      SUBROUTINE DMILV(IWD,IV,XV,KIND,IERR)
C
      IMPLICIT REAL*8 (A-H,O-Z)
C
      INTEGER*4 IWD(2)
...
C
      ...
      CALL ILBYTE(IT,IWD,IA-1)
      IF(IT.GE.'30'X.AND.IT.LE.'39'X) GO TO 100
      IF(IT.EQ.'2B'X.OR.IT.EQ.'2D'X)  GO TO 20
      IF(IT.EQ.'2E'X)                 GO TO 30
      GO TO 200
C
  20  IA=IA+1
      IF(IA.GT.8) GO TO 200
      GO TO 100
C
  30  IA=IA+1
      IF(IA.GT.8) GO TO 200
...
C
 100  DO 120 I=IA,8
      ...
      IF(IT.EQ.'2E'X) GO TO 110
      IF(IT.LT.'30'X.OR.IT.GT.'39'X) GO TO 200
      GO TO 120
 110  NDEC=NDEC+1
 120  CONTINUE
      IF(NDEC.GT.1) GO TO 200
